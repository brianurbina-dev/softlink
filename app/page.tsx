...
export default async function HomePage() {
  const [productosRaw, integrantesRaw, testimoniosRaw, articulosRaw] = await Promise.all([
    prisma.producto.findMany({ where: { activo: true }, orderBy: { creadoEn: "asc" } }),
    prisma.integrante.findMany({ where: { activo: true }, orderBy: { orden: "asc" } }),
    prisma.testimonio.findMany({ where: { activo: true }, orderBy: { orden: "asc" } }),
    prisma.articulo.findMany({ where: { publicado: true }, orderBy: { publicadoEn: "desc" }, take: 3 }),
  ]);
